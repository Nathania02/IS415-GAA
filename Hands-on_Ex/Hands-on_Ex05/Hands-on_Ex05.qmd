...
In this exercise, I will learn how to compute spatial weights using R. By the end to this hands-on exercise, you will be able to:
...
Contiguity weight matrices for study area can be computed using poly2nb() of spdep package. This function builds a neighbours list based on regions with contiguous boundaries.

The documentation shows that you can pass a "queen" argument that takes TRUE or FALSE as options. If the argument is not specified, the default is set to TRUE, and it will return a list of first order neighbours using the Queen criteria.
...
In this section, I will use dnearneigh() of spdep package to derive distance based weight matrices.
...
In this section, I will derive a spatial weight matrix based on Inversed Distance method. I will compute the distances between areas by using nbdists() of spdep.
...
The zero.policy=TRUE option allows for lists of non-neighbors. This should be used with caution since the user may not be aware of missing neighbors in their dataset however, a zero.policy of FALSE would return an error. To see the weight of the first polygon's eight neighbors type:
...
In this section, I will create four different spatial lagged variables. They are:
