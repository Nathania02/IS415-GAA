---
title: "Hands-on Ex 9: Calibrating Hedonic Pricing Model for Private Highrise Property with GWR Method"
author: "Nathania Yeo"
...
# 4.0 Import Data into R and Data Wrangling

## 4.1 Importing geospatial data

The geospatial data used in this hands-on exercise is called MP14_SUBZONE_WEB_PL. The GIS data is in svy21 projected coordinated systems.

```{r}
mpsz = st_read(dsn = "data/geospatial", layer = "MP14_SUBZONE_WEB_PL")
```

It is important to note that this object does not have EPSG information. Therefore, I will update mpsz with the correct EPSG code.
...
Currently, the *condo_resale* tibble data frame is aspatial. I will convert it to a **sf** object. The code chunk below converts condo_resale data frame into a simple feature data frame by using `st_as_sf()` of **sf** packages.
...
In this section, I will use statistical graphics function of ggplot2 package to perform EDA.
...
In this section, I will learn how to draw a small multiple histograms (also known as trellis plot) by using `ggarrange()` of [**ggpubr**](https://cran.r-project.org/web/packages/ggpubr/index.html) package.
...
In this section, I will build hedonic pricing models for condominium resale units using [`lm()`](https://www.rdocumentation.org/packages/stats/versions/3.5.2/topics/lm) of R base.
...
Since p-value is much smaller than 0.0001, I will reject the null hypothesis that mean is a good estimator of SELLING_PRICE. This will allow me to infer that simple linear regression model above is a good estimator of *SELLING_PRICE*.

The **Coefficients:** section of the report reveals that the p-values of both the estimates of the Intercept and ARA_SQM are smaller than 0.001. In view of this, the null hypothesis of the B0 and B1 are equal to 0 will be rejected. As a results, I will be able to infer that the B0 and B1 are good parameter estimates.

To visualise the best fit curve on a scatterplot, I can incorporate `lm()` as a method function in ggplot’s geometry as shown in the code chunk below.
...
### 6.2.1 Visualising the relationships of independent variables
...
With reference to the report above, it is clear that not all the independent variables are statistically significant. I will revised the model by removing those variables which are not statistically significant. Now, we are ready to calibrate the revised model by using the code chunk below.
...
### 6.5.4 Test for Spatial Autocorrelation
...
Next, I join the newly created data frame with condo_resale.sf object.
...
Next, I will convert *condo_resale.res.sf* from simple feature object into a SpatialPointsDataFrame because spdep package can only process sp conformed spatial data objects.
